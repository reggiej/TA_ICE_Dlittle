...
public class AggregateCollectionMapping extends CollectionMapping implements RelationalMapping {
    ...
    public AggregateCollectionMapping() {
        ...
        this.targetForeignKeyFields = oracle.toplink.internal.helper.NonSynchronizedVector.newInstance(1);
     ...
     * changeRecord
     * @return oracle.toplink.internal.sessions.AggregateCollectionChangeRecord the changerecord representing this AggregateCollectionMapping
     * @param owner oracle.toplink.internal.sessions.ObjectChangeSet the ChangeSet that uses this record
     * @param cloneCollection Object the collection to convert
     * @param session oracle.toplink.internal.sessions.AbstractSession
     */
    ...
    public void initializeChildInheritance(ClassDescriptor parentDescriptor, AbstractSession session) throws DescriptorException {
        ...
        if (parentDescriptor.getInheritancePolicy().hasChildren()) {
            ...
            Vector cloneChildDescriptors = oracle.toplink.internal.helper.NonSynchronizedVector.newInstance();
    ...
    public void initializeParentInheritance(ClassDescriptor parentDescriptor, ClassDescriptor childDescriptor, AbstractSession session) throws DescriptorException {
        ...
        Vector childern = oracle.toplink.internal.helper.NonSynchronizedVector.newInstance(1);
    ...
    protected void initializeTargetForeignKeyToSourceKeysWithDefaults(AbstractSession session) throws DescriptorException {
        ...
        setSourceKeyFields(oracle.toplink.internal.helper.NonSynchronizedVector.newInstance(sourceKeys));
        for (int index = 0; index < getTargetForeignKeyFields().size(); index++) {
            DatabaseField foreignKeyfield = getTargetForeignKeyFields().get(index);
            foreignKeyfield = getReferenceDescriptor().buildField(foreignKeyfield);
            getTargetForeignKeyFields().set(index, foreignKeyfield);
