...
import oracle.toplink.queryframework.SQLResultSetMapping;
...
public class Project implements Serializable, Cloneable {
    ...
    public void addDescriptor(ClassDescriptor descriptor, DatabaseSessionImpl session) {
        ClassDescriptor oldDescriptor = null;
        String alias = descriptor.getAlias();
        // Avoid loading class definition at this point if we haven't done so yet.
        if ((descriptors != null) && !descriptors.isEmpty()) {
            oldDescriptor = (ClassDescriptor)getDescriptors().get(descriptor.getJavaClass());
        }
        if ((oldDescriptor==null) && (alias != null) && (getAliasDescriptors()!=null) ) {
            oldDescriptor = (ClassDescriptor)getAliasDescriptors().get(alias);
        }
        //if this descriptor is the same as the oldDescriptor, it results in this descriptor being reinitilized.
        removeDescriptor(oldDescriptor, session);
    ...
    /**
     * INTERNAL: 
     * Unlink/remove a descriptor from the project. This clears the descriptor from the lists of
     * Aliases, OrderedDescriptors, the map of descriptors as well as its parent descriptor's list of
     * children if needed.
     * @param descriptor ClassDescriptor to remove and unlink from the project
     * @param session - DatabaseSessionImpl associated to the project
     */
    public void removeDescriptor(ClassDescriptor descriptor, DatabaseSessionImpl session){
        if (descriptor == null){
            return;
        }
        getOrderedDescriptors().remove(descriptor);
        String alias = descriptor.getAlias();
        if ( alias!=null && aliasDescriptors!=null ){
            aliasDescriptors.remove(alias);
        }
        
        if (descriptor.isChildDescriptor() ) {
            ClassDescriptor parentDescriptor = session.getDescriptor(descriptor.getInheritancePolicy().getParentClass());
            if (parentDescriptor!=null){
                parentDescriptor.getInheritancePolicy().getChildDescriptors().remove(descriptor);
            }
        }
    }
