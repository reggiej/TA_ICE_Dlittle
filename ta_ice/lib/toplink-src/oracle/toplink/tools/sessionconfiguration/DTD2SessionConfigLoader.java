...
public class DTD2SessionConfigLoader {
     ...
     * INTERNAL:
     */
    public TopLinkSessions load(Document document) {
        TopLinkSessions topLinkSessions = new TopLinkSessions();

        String publicId = document.getDoctype().getPublicId();
        String version = "unknown";

        if (publicId.equals(TopLinkEntityResolver.doctTypeId40)) {
            version = "4.0";
        } else if (publicId.equals(TopLinkEntityResolver.doctTypeId45)) {
            version = "4.5";
        } else if (publicId.equals(WASTopLinkEntityResolver.docTypeId_903)) {
            version = "9.0.3";
        } else if (publicId.equals(TopLinkEntityResolver.doctTypeId904) || publicId.equals(WASTopLinkEntityResolver.docTypeId_904)) {
            version = "9.0.4";
        }

        topLinkSessions.setVersion(version);

        NodeListElementEnumerator list = new NodeListElementEnumerator(document.getChildNodes());
        process_toplink_configuration_Tag(list.nextNode(), topLinkSessions);
     ...
     * INTERNAL:
     */
    protected void process_toplink_configuration_Tag(Node node, TopLinkSessions tlSessions) {
    ...
    */
    ...
    public void process_clustering_service_Tag(Node node, ObjectHolder holder) {
        ...
        if (serviceClass.equals("oracle.toplink.remote.rmi.RMIClusteringService")) {
            csmConfig.setClusteringServiceConfig(new RMIClusteringConfig());
        } else if (serviceClass.equals("oracle.toplink.remote.rmi.RMIJNDIClusteringService")) {
            csmConfig.setClusteringServiceConfig(new RMIJNDIClusteringConfig());
        } else if (serviceClass.equals("oracle.toplink.remote.rmi.wls.WLSClusteringService")) {
            csmConfig.setClusteringServiceConfig(new WLSClusteringConfig());
        } else if (serviceClass.equals("oracle.toplink.remote.rmi.iiop.RMIJNDIClusteringService")) {
            csmConfig.setClusteringServiceConfig(new RMIIIOPJNDIClusteringConfig());
        } else if (serviceClass.equals("oracle.toplink.remote.jms.JMSClusteringService")) {
            csmConfig.setClusteringServiceConfig(new JMSClusteringConfig());
        } else if (serviceClass.equals("oracle.toplink.remote.corba.sun.CORBAJNDIClusteringService")) {
    ...
    public void process_type_Tag(Node node, ObjectHolder holder) {
        if (node.getFirstChild().getNodeValue().equals("oracle.toplink.eis.EISLogin")) {
