...
public class SessionBroker extends DatabaseSessionImpl {
    ...
    public SessionBroker() {
        super(new oracle.toplink.sessions.DatabaseLogin());
    ...
    public SessionBroker acquireClientSessionBroker() {
        ...
        while (names.hasNext()) {
            ...
            if (ssession instanceof oracle.toplink.threetier.ServerSession) {
                if (ssession.getProject().hasIsolatedClasses()) {
                    throw ValidationException.isolatedDataNotSupportedInSessionBroker(sessionName);
                }
                clientBroker.internalRegisterSession(sessionName, ((oracle.toplink.threetier.ServerSession)ssession).acquireClientSession());
            } else {
     ...
     */
    public oracle.toplink.sessions.Session acquireHistoricalSession(AsOfClause clause) throws ValidationException {
     ...
     */
    public void addDescriptors(oracle.toplink.sessions.Project project) throws ValidationException {
        throw ValidationException.cannotAddDescriptorsToSessionBroker();
    }
    ...
    public boolean containsQuery(String queryName) {
        ...
        if (isClientSessionBroker() && (containsQuery == false)) {
            ...
            while (names.hasNext()) {
                sessionName = (String)names.next();
                ssession = getSessionForName(sessionName);
                if (ssession instanceof oracle.toplink.threetier.ClientSession) {
     ...
     * @see oracle.toplink.expressions.AsOfClause
     ...
     * The session should not be connected and descriptors should not be initialized.
     */
    public void registerSession(String name, oracle.toplink.sessions.Session session) {
