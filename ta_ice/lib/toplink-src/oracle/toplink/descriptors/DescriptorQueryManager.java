...
public class DescriptorQueryManager implements Cloneable, Serializable {
    ...
    public synchronized void addQuery(DatabaseQuery query) {
        ...
        if (queriesByName == null) {
            ...
            queriesByName = oracle.toplink.internal.helper.NonSynchronizedVector.newInstance();
            ...
            Vector argumentTypes = oracle.toplink.internal.helper.NonSynchronizedVector.newInstance(argumentTypesSize);
     ...
     * The call's query must be de-referenced in order to allow the GC of a related session.
     * PERF: Allow caching of the update SQL call to avoid regeneration.
     */
    public void putCachedUpdateCalls(Vector updateFields, Vector updateCalls) {
        ...
        if (!updateCalls.isEmpty()) {
            ...
            for (int i = 0; i < updateCallsSize; i++) {
                ...
                // clone call and de-reference query for DatasourceCall and EJBQLCall
