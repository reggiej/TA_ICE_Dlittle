...
import oracle.toplink.descriptors.copying.*;
import oracle.toplink.descriptors.changetracking.*;
...
//import oracle.toplink.internal.weaving.PersistenceWeavedChangeTracking;
 ...
 * @see oracle.toplink.eis.EISDescriptor
 * @see oracle.toplink.ox.XMLDescriptor
 */
public class ClassDescriptor implements Cloneable, Serializable {
    ...
    public void addCascadeLockingPolicy(CascadeLockingPolicy policy) {
        ...
        if (usesOptimisticLocking() && getOptimisticLockingPolicy().isCascaded()) {
            // 6874038: propagate later version changes up to the locking policy on a parent branch by setting the policy on all children here            
     ...
     *
     * Also note that the {@link oracle.toplink.sessions.UnitOfWork} will not refresh its registered objects.<P>
     *
     ...
     * if you require fresh data, it is better to configure a query with {@link oracle.toplink.queryframework.ObjectLevelReadQuery#refreshIdentityMapResult}.
     ...
     * query in a {@link oracle.toplink.remote.RemoteSession}.<P>
     *
     * However, if a query hits the cache, data is not refreshed regardless of how this setting is configured. For example, by
     * default, when a query for a single object based on its primary key is executed, OracleAS TopLink will first look in the
     * cache for the object. If the object is in the cache, the cached object is returned and data is not refreshed. To avoid
     ...
     * Normally, if you require fresh data, it is better to configure a query with {@link oracle.toplink.queryframework.ObjectLevelReadQuery#refreshIdentityMapResult}.
     * To ensure that refreshes are only done when required, use this method in conjunction with {@link #onlyRefreshCacheIfNewerVersion}.
     *
    ...
    public void convertClassNamesToClasses(ClassLoader classLoader){
        ...
        try{
            if (PrivilegedAccessHelper.shouldUsePrivilegedAccess()){
                ...
                descriptorClass = oracle.toplink.internal.security.PrivilegedAccessHelper.getClassForName(getJavaClassName(), true, classLoader);
            }
            if (getAmendmentClassName() != null){
                if (PrivilegedAccessHelper.shouldUsePrivilegedAccess()){
                    ...
                    amendmentClass = oracle.toplink.internal.security.PrivilegedAccessHelper.getClassForName(getAmendmentClassName(), true, classLoader);
                }            
            }
     ...
     * database by any query in a {@link oracle.toplink.remote.RemoteSession}.
     *
     * @see #alwaysRefreshCacheOnRemote
     */
    ...
    public String getAlias() {
        ...
        if ((alias == null) && (getJavaClassName() != null)) {
            alias = oracle.toplink.internal.helper.Helper.getShortClassName(getJavaClassName());
        }
        return alias;
    }
    ...
    public DescriptorEventManager getEventManager() {
        ...
        if (eventManager == null) {
            setEventManager(new oracle.toplink.descriptors.DescriptorEventManager());
        }
        return eventManager;
    }
    ...
    public InheritancePolicy getInheritancePolicy() {
        if (inheritancePolicy == null) {
            ...
            setInheritancePolicy(new oracle.toplink.descriptors.InheritancePolicy(this));
        }
    ...
    public DescriptorQueryManager getQueryManager() {
        ...
        if (queryManager == null) {
            setQueryManager(new oracle.toplink.descriptors.DescriptorQueryManager());
        }
        return queryManager;
    }
...
*/
     ...
     * <LI>the query was configured by calling {@link oracle.toplink.queryframework.ObjectLevelReadQuery#refreshIdentityMapResult}, or</LI>
     * <LI>the query was a call to {@link oracle.toplink.sessions.Session#refreshObject}</LI>
     * </UL>
     * <P>
     *
     ...
     *
     * Also note that the {@link oracle.toplink.sessions.UnitOfWork} will not refresh its registered objects.
     *
    ...
    */
     ...
     * the database by any query in a {@link oracle.toplink.remote.RemoteSession}.
     *
     ...
     * not desired. Normally, if you require fresh data, it is better to configure a query with {@link oracle.toplink.queryframework.ObjectLevelReadQuery#refreshIdentityMapResult}.
     * To ensure that refreshes are only done when required, use this method in conjunction with {@link #onlyRefreshCacheIfNewerVersion}.<P>
     *
     ...
     * is received from the database by any query in a {@link oracle.toplink.remote.RemoteSession}.
     *
     * @see #alwaysRefreshCacheOnRemote
     * @see #dontAlwaysRefreshCacheOnRemote
     */
     ...
     * refresh the cache if data is received from the database by any query in a {@link oracle.toplink.remote.RemoteSession}.
     ...
     * @see oracle.toplink.queryframework.ObjectLevelReadQuery#shouldRegisterResultsInUnitOfWork
     * @bug 2612601
     */
     ...
     * @see oracle.toplink.queryframework.FetchGroupTracker
     */
