...
import oracle.toplink.internal.helper.DateFormatThreadLocal;
...
public abstract class AbstractSessionLog implements SessionLog, java.lang.Cloneable {
     ...
     * external use when logging messages are required within the TopLink output.
     * </p><p>
     *
     * @param level the log request level value
     * </p><p>
     * @param message the string message - this shout not be a bundle key
     * </p>
     */
    public void log(int level, String message) {
		// Warning: do not use this function to pass in bundle keys as they will not be processed
        if (!shouldLog(level)) {
            return;
        }
    ...
    protected String getSessionString(Session session) {
        ...
        if (session != null) {
            return ((oracle.toplink.internal.sessions.AbstractSession)session).getLogSessionString();
        } else {
            return "";
        }
    }
    ...
    protected String formatMessage(SessionLogEntry entry) {
        ...
        if (entry.shouldTranslate()) {
            ...
            if (entry.getParameters()!=null && entry.getParameters().length>0 && message.indexOf("{0") >= 0) {
            	...
            	/*
				 * Bug6891149 Look for message ID in TraceLocalization (untranslated at FINE, FINER, FINEST levels)
            	 * If the key is already transformed to a value containing {N} then do not use it as a key
				 * For cases where a message is a single word - we cannot differentiate whether it is a key or value
            	 */
